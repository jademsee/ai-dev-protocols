...
---
## /test
Red-green-refactor loop. Drive by test suite only.
Run the improvement loop on the current file.
Drive entirely by the test suite: fix RED, then refactor GREEN.
Do not make any change that causes a previously passing test to fail.
Follow Hard Stops from global_rules.md.
Wait for my approval before each iteration.

**Test Framework Detection:** Before first test run, detect framework using this hierarchy.
...
Read existing CI config (`.github/workflows/*.yml`, `.gitlab-ci.yml`, `Jenkinsfile`,
`azure-pipelines.yml`) for actual test commands being used.
