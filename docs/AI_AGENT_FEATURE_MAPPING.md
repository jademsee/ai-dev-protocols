...
Our skills system provides 14 specialized capabilities organized into Atomic, Composite, and Orchestration categories.

### Windsurf/Cascade Configuration

**Location:** `skills/` directory (14 subdirectories)

**Skill Categories:**
- **Atomic**: debug, write-tests, write-docs, refactor, optimize, maintain-consistency, manage-dependencies, manage-secrets
...
|----------------|-----------------|---------------|--------|----------|-----------|-------------|-------------|-------|------------|-------|
| **Atomic Skills** | `skills/` (8 custom) | Not supported | File-specific rules in `.cursor/rules/` | Custom slash commands in `config.yaml` | 5 built-in modes + custom modes | Built-in + custom agents | Skills in `.claude/skills/` | Not supported | MCP tools | Not supported |
